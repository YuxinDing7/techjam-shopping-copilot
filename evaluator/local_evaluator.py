...
"""Local conversation simulator and scorer.

The evaluator, rather than the Agent, controls the customer conversation:
it creates an initial scenario-specific message, sends the Agent response to
the deterministic customer policy, and repeats for at most ten turns. The
Agent's structured ``ask_attribute`` field determines which constraint the
customer may reveal next; the natural-language ``message`` does not control
the simulator. Intent-override sessions replace the old preference on turn
three or four before recommendations are scored.
"""
...
def initial_message(sample: dict, category: str, disclosed: set[str]) -> str:
    """Create the customer's first message for the selected scenario.

    Buying reveals one hard constraint early. Intent override starts with an
    old soft preference. Browsing and boundary sessions start exploratory and
    do not reveal a concrete preference here.
    """
...
def customer_reply(sample: dict, ask_attribute: object, disclosed: set[str], boundary_used: bool) -> tuple[str, bool]:
    """Generate the next customer turn from the Agent's structured question.

    The simulator reads ``ask_attribute`` directly. It reveals up to two
    undisclosed matching constraints, reports no preference when none match,
    and uses a one-time no-preference response in boundary sessions.
    """
...
def evaluate(
    ...
    categories: dict[str, list[str]],
    products: dict[str, dict],
    conversation_log: str | Path | None = None,
) -> dict:
    sessions: list[dict] = []
    ...
    try:
        for sample in samples:
            ...
            # Each turn scores the current recommendations before asking the
            # simulated customer for another reply.
                try:
                ...
                if log_handle is not None:
                    ...
                    try:
                        agent_debug = debug_snapshot(session_id) if callable(debug_snapshot) else {}
                    except Exception:
                        agent_debug = {}
                    log_handle.write(json.dumps({
                        ...
                        "normalized_recommendations": ranked,
                        "target_visible_to_evaluator": target,
                        "hit": hit,
                ...
                if log_handle is not None:
                    log_handle.write(json.dumps({
                        ...
                        "sample_id": sample["sample_id"],
                        "next_turn": turn + 1,
                        "simulated_user_message": user_message,
                    }, ensure_ascii=False) + "\n")
                    log_handle.flush()
            sessions.append({
                "sample_id": sample["sample_id"],
...
def main() -> None:
    ...
    parser.add_argument("--conversation-log", default=None)
